        ...
        target.load = function(parms){
            ...
            if (options.url) {
                ...
                if (data && data.length > 0) {
                    ...
                    target.find("tbody").find("tr").click(function () {
                        if (hasSelectItem) {
                            ...
                            if (_ipt.attr("type") == "radio") {
                                ...
                                /*此处兼容icheck 控件*/
                                $("input[name='select_item']").parent().removeClass("checked");
                                $(_ipt).parent().addClass("iradio_square-blue checked");
                                return false;
                            } else {
                                if (_ipt.prop('checked')) {
                                    _ipt.prop('checked', false);
                    ...
                    var _errorMsg = '<tr><td colspan="' + options.columns.length + '"><div style="display: block;text-align: center;">未获取到相关数据!</div></td></tr>'
                    tbody.html(_errorMsg);
                }
                //加载已有数据 end
            }
